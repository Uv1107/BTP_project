...
screen.fill(WHITE)
myfont = pygame.font.SysFont("lucida grande", 19)
# label1 = myfont.render(
    ...
    for event in pygame.event.get():
            ...
            with open(path) as dataFile:
                for line in dataFile:
                    ...
                    if not intersection:
                        ...
                        if len(pts) > 1:
                            pygame.draw.aaline(
                                screen, BLUE, pts[len(pts) - 2][:2], currPt)
                    else:
                        break
                ...
                if not intersection:
                    pygame.draw.aaline(
                        screen, BLUE, pts[len(pts) - 1][:2], pts[0][:2])
                    lock = 1
                else:
                    screen.fill(WHITE)
                    ...
                    pygame.draw.aaline(
                        screen, BLACK, (0, 25), (WINDOW_WIDTH, 25))
                    # pygame.draw.aaline(
                    #     screen, BLACK, (0, WINDOW_HEIGHT - 33), (WINDOW_WIDTH, WINDOW_HEIGHT - 33))
                    # displayLabel1(
                    #     "Place first point (LEFT-MOUSE-CLICK), Load Polygon from file (SPACE),")
                    # displayLabel2("Reset (BACKSPACE)")
                    displayStatus(
        ...
        if event.type == MOUSEBUTTONDOWN and pygame.mouse.get_pressed()[0] and lock != 1:
            ...
            if not intersection:
                ...
                if len(pts) > 1:
                    pygame.draw.aaline(
                        screen, BLUE, pts[len(pts) - 2][:2], mpos)
            else:
                displayStatus(
                    "Cannot insert this vertex as it doesn't produce a simple polygon. Please try again.")
        ...
        if event.type == KEYDOWN and pygame.key.get_pressed()[K_RETURN] and lock == 1:
            ...
            stepsFinish = 1
            displayVariation(
                variations[validColors[currVariationDisplayed]], guardSelected)
        ...
        if event.type == KEYDOWN and pygame.key.get_pressed()[K_RIGHT] and lock == 1 and stepsFinish == 1:
            currVariationDisplayed = (
                currVariationDisplayed + 1) % len(validColors)
            guardSelected = 0
            displayVariation(
                variations[validColors[currVariationDisplayed]], guardSelected)
        ...
        if event.type == KEYDOWN and pygame.key.get_pressed()[K_UP] and lock == 1 and stepsFinish == 1:
            guardSelected = (
                guardSelected-1) % (len(variations[validColors[currVariationDisplayed]])+1)
            displayVariation(
                variations[validColors[currVariationDisplayed]], guardSelected)
        ...
        if event.type == KEYDOWN and pygame.key.get_pressed()[K_BACKSPACE]:
            ...
            displayStatus(
                "Start by building a polygon or loading a polygon from a file")
            # displayLabel1(
            #     "Place first point (LEFT-MOUSE-CLICK), Load Polygon from file (SPACE)")
    pygame.display.update()
